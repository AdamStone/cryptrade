...
from utilities import TRADES, CANDLES, ut_to_dt, dt_to_ut, build_data_directories, \
                      parse_period, pdelta, get_candle, trades_to_candles, save_candlefile
...
class TradeStream(object):
    ...
    def __init__(self, market='bitfinex_BTC_USD', record_trades=True, quiet=False):
        ...
        self.exchange, self.base, self.alt = (item.lower() for item in market.split('_'))
        ...
        if response:
            ...
            if new_trades:
                ...
                for trade in new_trades:
                    if not self.quiet:
                        print "{}   {}   {} {}   {} {}".format(ut_to_dt(trade['timestamp']), 
                            self.exchange, trade['price'], self.alt, trade['amount'], self.base)
...
class CandleStream(object):
    ...
    def get_candles_since(self, timestamp): # candles beginning after timestamp
...
class ExchangeTrader(object):
    ...
    def __init__(self, candlestream, api, strategy):
        ...
        self.exchange, self.base, self.alt = candlestream.exchange, candlestream.base, candlestream.alt
        ...
        if response == None:
            ...
            for entry in response:
                ...
                if orderType not in self.finances:
                    self.finances[orderType] = {entry['currency']: {'available': entry['available'], 'amount': entry['amount']}}
                else:
                    self.finances[orderType][entry['currency']] = {'available': entry['available'], 'amount': entry['amount']}
            return True
    ...
    def getMarketPrice(self, action=None):
        response = self.api.book({'limit_bids': 5, 'limit_asks': 5}, symbol=self.symbol)
    ...
    def placeOrder(self, action, amount='all', orderType='exchange market', price=None):
        """ Example response:
       {u'avg_execution_price': u'0.0', 
        ...
        u'side': u'sell'}
        """
        print 'placeOrder triggered with action {}, amount {}, type {} and price {}'.format(action, amount, orderType, price)
        if not price:
            price = self.getMarketPrice(action)
            if price:
                print 'price not provided; market price of {} used'.format(price)
            else: 
        ...
        if amount in ['all', 'All']:
            if action == 'Sell': 
                ...
                print '{} sell all attempted with order amount {}'.format(orderType, amount)
        ...
        if reference == 'peak':
            ...
            if self.my_trades:
                if self.position() == 'long':
                    ...
                    candles_from = self.candlestream.get_candles_from(timestamp)
                    max_from = Decimal(max([entry_price] + [c[2] for c in candles_from]))
    ...
    def update(self):
        ...
        if self.position() == 'long':
            # check if expected stoploss is missing or stop order should be increased
            ...
            for order in self.openOrders:
                if order['side'] == 'sell' and 'stop' in order['type']:                 
                    ...
                    if new_exit > current_exit:
                        print 'exit point has increased from {} to {} since stop order placed; resubmitting stop order'.format(current_exit, new_exit)
                        # cancel current stop order
                        self.queue.append(lambda: self.cancelOrder(order['id']))
                        self.queueRequery()
                        # post new stop order at higher price
                        self.queue.append(lambda: self.placeOrder('Sell', orderType='exchange stop', price=new_exit))
                        self.queueRequery()
                        return actionTaken
                    
            if missing and 'stop' not in self.lastTradeType():
                print 'stoploss is missing; queuing stop order'
                self.queue.append(lambda: self.placeOrder('Sell', orderType='exchange stop', price=self.exitPoint()))
                self.queueRequery()
                return actionTaken
        ...
        action = self.strategy.check(position=self.position(), my_trades=self.my_trades, candlestream=self.candlestream) # **kwargs
        if action:
    ...
    def trade(self, action, amount=None):
        ...
        if action == 'Buy':
            if not amount:
                ...
                amount = dollars_at_risk / price_move * (1 - self.strategy.commission)   # buy amount based on risk management
            ...
            # make sure sufficient finances are available
                ...
                self.messages.append('WARNING: cost of buy exceeds available finances.')
                return
            ...
            print 'queuing stop sell order' # using same amount here causes 'not enough balance' error due to commission loss
            self.queue.append(lambda: self.placeOrder('Sell', orderType='exchange stop', price=self.exitPoint()))
            self.queueRequery()
