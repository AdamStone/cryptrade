...
class MainWindow(QtGui.QMainWindow):
    ...
    def __init__(self, record_trades=True, record_candles=True):
        ...
        self.marketOptions = ['bitfinex_BTC_USD']
        self.periodOptions = ['5 minute', '10 minute', '15 minute',
                              '30 minute', '1 hour', '2 hour', '6 hour',
                              '12 hour', '24 hour']

        self.candleStreams = {}
        for period in self.periodOptions:
            self.candleStreams[period] = CandleStream(
                self.tradestream, period,
                record_candles=record_candles, quiet=True)
    ...
    def createMainFrame(self):
        ...
        self.period.currentIndexChanged[str].connect(
            lambda: self.delayedPlot(2000))
    ...
    def updateTextBox(self, new_trades):
            for trade in self.tradestream.new_trades:
                self.textBox.append("{}   {}   {} {}   {} {}".format(
                    ut_to_dt(trade['timestamp']), self.tradestream.exchange,
                    trade['price'], self.tradestream.alt, trade['amount'],
                    self.tradestream.base))
            self.textBox.moveCursor(QtGui.QTextCursor.End)
    ...
    def updatePlot(self):
        ...
        # checked indicators here are calculated within plot()
        plotIndicators = [indc[0] for indc in self.indicators
                          if indc[1].checkState()]
        try:
            self.figure.plot(candles, period, indicators=plotIndicators,
                             ncandles=int(len(candles)/2))
        except:
    ...
    def update(self):
        ...
        # update traderWindow regardless of new trades,
        # to attempt to process queue
